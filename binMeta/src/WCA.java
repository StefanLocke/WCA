...
public class WCA extends binMeta {
	 ...
	 */
	public WCA(Data startPoint, Objective Obj, long maxTime,int numberOfSeaAndRivers,int numberOfTotal) {
		
		 try
	      {
	        String msg = "Impossible to create WCA object: ";
	        ...
	        if (numberOfSeaAndRivers <= 0) throw new Exception(msg + "the number of Seas and Rivers gien is 0 or even negative");
	        if (numberOfTotal <= 0) throw new Exception(msg + "the number of total Streams given is 0 or even negative");
			...
			generate(numberOfSeaAndRivers, numberOfTotal);
			designateAll();
			assign();
	      }
		 catch (Exception e){
	...
	private List<Stream> pop; //list of all the streams
	private List<Thread> workers; //list of all the workers
	private Semaphore S = new Semaphore(1); //semaphore for accessing obj (because of the way certain obj are coded)
	//we could also potentially use synchronised on a method that calls obj.value
	...
	public void generate(int numberOfSeaAndRivers, int numberOfTotal) { //generates a random sample of streams and sorts them to rivers or seas
		...
		for (int i = 0; i < numberOfTotal; i++) {
			...
			pop.add(new Stream(this.solution.randomSelectInNeighbour(2))); //more ways than one to do it here
		}
		pop.sort(null);
...
//---------------------------------------------------------------------------------------------		
	...
	public void optimize() {
		...
		while (System.currentTimeMillis() - startTime < maxTime && obj.value(this.solution) > 0) {
			moveStreams();
			swapStreams();
			moveRivers();
			swapRivers();
			this.solution = pop.get(0).data;
			i++;
	...
	public static void main(String[] args) {
		...
		int ITMAX = 20000;  // number of iterations
	      ...
	      Data D = obj.solutionSample();
	      WCA rw = new WCA(D,obj,ITMAX,10,20);
